...
trait MorphirJsonEncodingSupportV1 {
  ...
  implicit def packageSpecificationEncoder[TA: JsonEncoder]: JsonEncoder[PackageModule.Specification[TA]] =
    Json.encoder.contramap[PackageModule.Specification[TA]] { specification =>
      Json.Obj(
  ...
  implicit def packageDefinitionEncoder[TA: JsonEncoder, VA: JsonEncoder]
      : JsonEncoder[PackageModule.Definition[TA, VA]] =
    Json.encoder.contramap[PackageModule.Definition[TA, VA]] { definition =>
      Json.Obj(
