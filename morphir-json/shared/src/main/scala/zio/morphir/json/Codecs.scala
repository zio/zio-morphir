  ...
  implicit def patternEncoder[Annotations](implicit
    ...
    pattern match {
      ...
      case Pattern.TuplePattern(patterns, annotations) =>
        Json.Arr(Json.Str("tuple_pattern"), toJsonAstOrThrow(annotations), Json.Arr(patterns.map(toJsonAstOrThrow(_))))
  ...
  implicit def valueEncoder[Annotations](implicit
    ...
    Json.encoder.contramap[Value[Annotations]] { value =>
      value.foldAnnotated[Json] {
        ...
        case (ValueCase.LambdaCase(argumentPattern, body), annotations) =>
          Json.Arr(Json.Str("lambda"), toJsonAstOrThrow(annotations), ???, body)
        case (ValueCase.LetDefinitionCase(valueName, valueDefinition, inValue), annotations) =>
          Json.Arr(Json.Str("let_definition"), toJsonAstOrThrow(annotations), toJsonAstOrThrow(valueName), ???, inValue)
        case (ValueCase.LetRecursionCase(valueDefinitions, inValue), annotations) =>
          Json.Arr(Json.Str("let_recursion"), toJsonAstOrThrow(annotations), ???, inValue)
        case (ValueCase.DestructureCase(pattern, valueToDestruct, inValue), annotations) =>
        ...
        case (ValueCase.PatternMatchCase(branchOutOn, cases), annotations) =>
        ...
        case (ValueCase.NativeApplyCase(nativeFunction, arguments), annotations) =>
