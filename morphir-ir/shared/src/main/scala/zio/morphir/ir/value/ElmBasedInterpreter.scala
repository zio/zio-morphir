...
object ElmBasedInterpreter {
  ...
  def evaluateFunctionValue(
      ...
      ir: IR,
      name: FQName,
      variables: Chunk[RawValue]
  ): Either[Error, RawValue] = ???

  def evaluate(nativeFunctions: Map[FQName, NativeFunction], ir: IR, value: RawValue): Either[Error, RawValue] = ???
        ...
        for {
          ...
          result <- {
            evaluatedRecord.caseValue match {
              case RecordCase(oldFields: Chunk[(Name, RawValue)]) =>
                Right(RecordCase[RawValue](oldFields ++ evaluatedFields))
  ...
  def matchPattern(
    ...
    def helper(values: Chunk[RawValue], patterns: Chunk[RawValue]): MatchResult = {
      if (values.length != patterns.length) then noMatch
      else {
