...
trait LiteralGen extends WordGen {
  final def asciiCharLiteral: Gen[Any, Literal.Char] = Gen.asciiChar.map(Literal.Char)

  final def boolLiteral: Gen[Any, Literal.Bool] = Gen.boolean.map(Literal.Bool)

  final def charLiteral: Gen[Any, Literal.Char]                       = Gen.char.map(Literal.Char)
  final def charLiteral(min: Char, max: Char): Gen[Any, Literal.Char] = Gen.char(min, max).map(Literal.Char)
  ...
  def stringLiteral: Gen[Any, Literal.String] = words.map(Literal.String)
